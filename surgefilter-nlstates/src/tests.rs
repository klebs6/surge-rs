/// These tests verify that the `NLSFType`, `NLSFCoeff`, `NLSFState`, and `NLSFSaturator`
/// structures are correctly defined and that the `process_quad` function works as expected for
/// a specific input and filter state. 
///
/// You could add more tests to cover other cases and edge cases as well.
///
/// Here are some additional test cases that could be added:
/// 
/// 1. Test edge cases for the `NLSFSaturator::SoftClip` option by passing in very small and very
/// large input values and ensuring that the output is clipped to the appropriate range.
/// 
/// 2. Test the behavior of the filter when `stages` is set to 0 (i.e., no stages). In this case,
/// the output should be the same as the input, and none of the coefficient or state variables
/// should be modified.
/// 
/// 3. Test the behavior of the filter when `stages` is set to its maximum value (i.e., all four
/// stages). In this case, the output should be the result of processing the input through all four
/// stages, and all of the coefficient and state variables should be updated accordingly.
/// 
/// 4. Test the behavior of the filter when the `qfu` argument has been initialized with non-zero
/// values for the `z1` and `z2` state variables. In this case, the filter should pick up where it
/// left off from the previous call to `process_quad`, and the output should reflect the state of
/// the filter after processing the new input.
/// 
/// 5. Test the behavior of the filter when the `qfu` argument has been initialized with non-zero
/// values for the `coeff` array. In this case, the filter coefficients should be updated before
/// processing the input, and the output should reflect the effect of these updated coefficients on
/// the filtering operation.
/// 
/// 6. Test the performance of the filter by running it on a large input signal and measuring the
///    processing time. This can be useful for determining whether the filter is suitable for
///    real-time applications or whether optimizations are needed to improve its performance.
///
#[cfg(test)]
mod tests {
    use super::*;
