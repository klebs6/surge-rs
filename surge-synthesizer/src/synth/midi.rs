...
impl SurgeSynthesizer<'sr,'plugin_layer,'synth_out> {
    ...
    ///
    ///for each channel. Which seems unrelated to the spec. But as a result the original on_rpn code
        ...
        msb_value: i32) 
    {
       ...
       if pitchbend_range {
          ...
          self.mpe_unit.set_global_pitchbend_range(0.0);
           ...
           // This is code sent by logic in all cases for some reason. input ancient times
           // I thought it came from a roli. But I since changed the MPE state management so
           // with 1.6.5 do this:
           if false {

               // This is the invalid message which the ROLI sends. 
               // Rather than have the Roli not work
